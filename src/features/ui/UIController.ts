...
export class UIController {
  ...
  showBoosterModal(isFirstTime: boolean = true): Promise<boolean> {
    return new Promise((resolve) => {
      ...
      if (this.boostScreen) {
        ...
        const acceptBtn = this.boostScreen.querySelector(
          ".game-booster__accept",
        );
        ...
        if (acceptBtn) {
          ...
          const handleAccept = (event?: Event) => {
            ...
            this._cleanupBoosterModalListeners(
              acceptBtn as HTMLElement,
              handleAccept,
              handleKeyboard,
            );
            resolve(true);
          ...
          console.warn(
            "Booster accept button (.game-booster__accept) not found",
          );
          resolve(true);
        }
      } else {
  ...
  launchConfetti(): void {
    try {
      if (!this.confettiManager) {
        ...
        if (!confettiCanvas) {
          console.warn(
            "Confetti canvas (#confetti-canvas) not found, using default",
          );
        }
  ...
  showTutorialHint(): Promise<void> {
    return new Promise((resolve) => {
      ...
      const baseUrl =
        typeof window !== "undefined" && (window as any).GAME_ASSETS_URL
          ? (window as any).GAME_ASSETS_URL
          : "";
      const animationPath = baseUrl
        ? `${baseUrl}/assets/animations/tutorial-hint.json`
        : "/assets/animations/tutorial-hint.json";
    ...
    return new Promise((resolve) => {
      ...
      const baseUrl =
        typeof window !== "undefined" && (window as any).GAME_ASSETS_URL
          ...
          : "";
      const animationPath = baseUrl
        ? `${baseUrl}/assets/animations/booster-activation.json`
        : "/assets/animations/booster-activation.json";
